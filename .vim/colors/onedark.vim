...
let s:gutter_bg_grey = { "gui": "#21252B", "cterm": "NONE", "cterm16": "NONE" }
...
call s:h("IncSearch", { "bg": s:visual_grey }) " 'incsearch' highlighting; also used for the text replaced with ":s///c"
...
call s:h("StatusLine", { "fg": s:white, "bg": s:gutter_bg_grey }) " status line of current window
call s:h("StatusLineNC", { "fg": s:gutter_fg_grey, "bg": s:gutter_bg_grey }) " status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
